...
public class LdapUserRolesProvider implements UserRolesProvider {
    ...
    public Pair<String, Collection<Role>> mapFromAttributes(Attributes attrs)
      ...
      if (memberOfAttribute != null) {
        for (NamingEnumeration<?> memberOf = memberOfAttribute.getAll(); memberOf.hasMore(); ) {
          ...
          String role = org.springframework.ldap.support.LdapUtils.getStringValue(ln, configProps.getGroupRoleAttributes());
          roles.add(new Role(role).setSource(Role.Source.LDAP));
        }
      }

      return Pair.of(
          attrs.get(configProps.getUserIdAttribute()).get().toString().toLowerCase(), roles);
    }
  }
  ...
  public Map<String, Collection<Role>> multiLoadRoles(Collection<ExternalUser> users) {
    ...
    return ldapTemplate.search(
            configProps.getUserSearchBase(), filter.toString(), new UserRoleMapper())
            .stream()
            .flatMap(p -> {
